		...
		try {
			const updateEmbeds = async () => {
				...
				const header = new EmbedBuilder()
					...
					.setDescription('Top live crypto prices on the market.')
				...
				for (let i = 0; i < Math.min(maxEmbeds, Math.ceil(coinGeckoData.length / maxFieldsPerEmbed)); i++) {
					...
					coinGeckoData.slice(i * maxFieldsPerEmbed, (i + 1) * maxFieldsPerEmbed).forEach(coin => {
						const price = coin.current_price % 1 === 0 ? coin.current_price : coin.current_price.toFixed(3);
						body.addFields({ name: `🪙 ${coin.symbol.toUpperCase()}`, value: `🏷️ $${price}`, inline: true });
					});
